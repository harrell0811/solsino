...
export default function BlackjackGame({ userId, balanceLamports, onBalanceChange }) {
  ...
  return <div className="panel blackjack-game"><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><h2 style={{ margin: 0, fontSize: 18 }}>Blackjack</h2><span className="mono" style={{ color: 'var(--text-muted)', fontSize: 12 }}>DEALER STANDS ON 17</span></div>
    <div className="blackjack-table"><div className="hand-label">Dealer {game?.dealerScore ? `· ${game.dealerScore}` : ''}</div><div className="card-hand">{game?.dealerCards?.map((c, i) => <Card key={i} card={c} delay={i * 120} />) || <div className="card-placeholder">?</div>}</div><div className="blackjack-divider" /><div className="hand-label">Your hand {game ? `· ${game.playerScore}` : ''}</div><div className="card-hand">{game?.playerCards?.map((c, i) => <Card key={i} card={c} delay={i * 120} />) || <div className="card-placeholder">?</div>}</div>
      {game?.complete && <div className={`blackjack-result ${game.outcome === 'win' || game.outcome === 'blackjack' ? 'result-win' : game.outcome === 'push' ? '' : 'result-lose'}`}>{game.outcome === 'blackjack' ? 'BLACKJACK!' : game.outcome === 'bust' ? 'BUST' : game.outcome?.toUpperCase()} {Number(game.payoutLamports) > 0 && `· ${lamportsToSol(game.payoutLamports)} SOL`}</div>}</div>
    {!game || game.complete ? <><label className="field-label">Wager (devnet SOL)</label><input type="number" min="0" step="0.001" value={wager} onChange={(e) => setWager(e.target.value)} style={{ width: '100%' }} /><div style={{ marginTop: 8 }}><QuickBetButtons wager={wager} setWager={setWager} balanceLamports={balanceLamports} disabled={loading} /></div><button className="btn btn-brand" style={{ width: '100%', marginTop: 16 }} disabled={!userId || loading} onClick={() => action(() => api.dealBlackjack(userId, solToLamports(wager)))}>{userId ? (loading ? 'Dealing…' : 'Deal') : 'Connect wallet to play'}</button></> : <div className="blackjack-actions"><button className="btn btn-positive" disabled={loading} onClick={() => action(() => api.hitBlackjack(userId, game.gameId))}>Hit</button><button className="btn btn-brand" disabled={loading} onClick={() => action(() => api.standBlackjack(userId, game.gameId))}>Stand</button></div>}
